...
// métodos lógicos de la clase rational_t:
    ...
    // comparaciones:
    bool rational_t::is_equal(const rational_t& r, const double precision = EPSILON) const {
        if (r.value() == 0) {
    ...
    bool rational_t::is_greater(const rational_t& r, const double precision = EPSILON) const {
        return ((value() - r.value()) > precision);
    }

    bool rational_t::is_less(const rational_t& r, const double precision = EPSILON) const {
